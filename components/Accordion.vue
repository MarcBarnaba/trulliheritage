<template>
  <div class="accordion">
    <div class="accordion-item" v-for="(item, index) in items" :key="index">
      <button @click="toggle(index)" class="w-full text-left py-2 text-lg transition-all duration-100 ease-in-out"
        :class="isActive(index) ? 'font-bold' : 'font-normal'">
        {{ item.title }}
      </button>

      <transition name="accordion">
        <div v-if="isActive(index)" class="pl-4 py-2 transition-all duration-150 ease-in-out accordion-content">
...
.accordion-content {
  ...
  /* Keeps content opening from the top */
}
</style>